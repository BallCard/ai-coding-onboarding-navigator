...
const RAW_ROADMAP_NODES: RoadmapNode[] = [
  ...
  },
  {
    ...
    level: 'starter',
    title: '检查电脑环境',
    description: '先证明这台电脑会用终端、能运行 Node、npm、Git，并具备访问官方服务的网络条件。三项没过，不安装 AI 工具。',
    userGoal: '跑完 4 条检查命令，判断能不能进入安装。 ',
    nextStepId: 'install-tool',
    tasks: ['选择 Windows/macOS', '检查终端', '检查 Node.js 与 Git'],
    successCriteria: ['看到 4 个结果：Node 版本号、npm 版本号、Git 版本号、npm registry 查询结果；同时确认当前网络能访问官方服务。缺任何一个，都先排障。'],
    commonPitfalls: 3,
    ...
    detail: {
      preflight: ['你已经选定本次工具。还没选工具就回到上一步。', '这一步不安装 Claude Code，也不安装 Codex。只确认终端、Node、npm、Git 和 npm 查询能力可用。', 'Claude Code 和 Codex 都依赖海外官方服务。你需要能稳定访问官方文档、登录页和模型服务；校内同学通常会把这类条件称为“科学上网”。本站只提醒你确认网络条件，不提供具体网络配置方案。'],
      steps: ['按你的系统打开终端。Windows 用 PowerShell，macOS 用 Terminal。', '复制第一段命令，粘贴到终端，按回车。', '确认 Node、npm、Git 三行都有版本号。', '复制第二段命令，确认 npm 能查到 registry 和版本号。', '用浏览器打开 Claude Code / Codex 官方文档，确认当前网络能访问官方服务。', '任何一条失败，都停在这里排障，不进入安装工具。'],
      commands: [
        {
          label: 'Windows 环境检查',
          command: 'node --version\nnpm --version\ngit --version',
          note: 'PowerShell：三条命令都能输出版本号即可继续；最低要求以对应官方安装文档为准',
        },
        {
          label: 'macOS 环境检查',
          command: 'node --version\nnpm --version\ngit --version',
          note: 'Terminal：三条命令都能输出版本号即可继续；最低要求以对应官方安装文档为准',
        },
        {
      ...
      differences: ['Windows 重点看 `not recognized`。出现这个，说明命令不存在或 PATH 没配好。', 'macOS 重点看 `command not found`。出现这个，说明命令不存在或 shell 没加载。', '浏览器能打开官方文档，只说明网页可达；后续 CLI 登录、npm 下载、模型请求仍可能分别失败。'],
